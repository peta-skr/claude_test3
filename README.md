...
It renders **images** (through its own PNG **and** JPEG decoders) and
**tables** too:

![Images and tables](docs/screenshot-features.png)

It runs **JavaScript** (this page's heading, list and total are generated by
a script) and draws **form controls**:

![JavaScript-generated content](docs/screenshot-js.png)
![Form controls](docs/screenshot-forms.png)
...
```
URL ─▶ Network ─▶ HTML Parser ─▶ JavaScript ─▶ CSS Engine ─▶ Layout ─▶ Paint ─▶ Raster ─▶ PNG
       (sockets)    (DOM tree)    (mutate DOM)   (cascade)    (boxes)  (display  (framebuffer)
       cookies/cache                                                   list)
```
...
| `pybrowser/layout.py` | Block + inline + **table** + **float**/**inline-block** layout: the box model, word wrapping, images, **form widgets**, list markers, links |
...
| `pybrowser/jpeg.py` | A **from-scratch baseline JPEG encoder + decoder** (Huffman, DCT, YCbCr) |
| `pybrowser/js.py`, `js_dom.py` | A small **JavaScript** interpreter + DOM bindings (`document.*`, elements, `console`) |
| `pybrowser/session.py` | A **cookie jar** and response **cache** shared across tabs |
| `pybrowser/browser.py` | Tabs, history, image loading, script execution, hit testing, Chrome-like chrome |
...
- **Layout**: block, inline, **table**, **float** (left/right) and
  **inline-block** formatting; the box model (margins, padding, borders); word
  wrapping; `width`/`height`; `margin:auto` centering; `text-align`;
  `text-decoration`; `display:none`; list markers; headings; `<hr>`; `<pre>`;
  and relative/`em`/`%` font sizes.
- **Images**: `<img>` with from-scratch **PNG** (grayscale/truecolor/palette,
  alpha, all five filters) **and baseline JPEG** (Huffman + DCT + YCbCr)
  decoders, nearest-neighbour scaling, alpha compositing, and a labelled
  placeholder for broken images.
- **Forms**: text/password/checkbox/radio/submit inputs, `<button>`,
  `<textarea>` and `<select>` render as real controls.
- **JavaScript**: a small interpreter (variables, functions, loops, arrays,
  objects, `String`/`Array`/`Math`/`JSON`) with DOM bindings —
  `document.getElementById`/`querySelector`/`createElement`, element
  `textContent`/`innerHTML`/`setAttribute`/`style`/`appendChild`,
  `console.log`. Scripts run at load and their DOM changes are laid out.
- **Networking**: `http`/`https` over raw sockets + TLS, `CONNECT`-proxy
  support, redirects, gzip, chunked transfer, plus a **cookie jar**
  (Domain/Path/Secure/expiry) and an in-memory **response cache**.
...
72 tests cover the parser, CSS cascade, URL handling, the font/rasterizer,
the PNG and JPEG codecs, image/table/float/inline-block layout, form widgets,
the JavaScript interpreter and DOM, cookies and caching, hit testing, and the
...
one. Its JavaScript is a useful subset (no prototypes/classes, regex, or
event loop); its CSS covers common flow layout but not flexbox or grid; its
image support is PNG and baseline JPEG (no progressive JPEG, GIF or WebP); and
there is no incremental/GPU compositing. The goal is a complete, readable
pipeline you can hold in your head — from a URL string all the way down to
individual pixels in a PNG.
